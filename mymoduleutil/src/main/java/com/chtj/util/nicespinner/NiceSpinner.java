...
/*
 * Copyright (C) 2018 Angelo Marchesin.
 *
...
public class NiceSpinner extends AppCompatTextView {
    ...
    private PopupWindow popupWindow;//NiceSpinner中的下拉弹窗
    private ListView listView;//PopuWindow中显示的ListView
    private NiceSpinnerBaseAdapter adapter;//NiceSpinner适配器
    private AdapterView.OnItemClickListener onItemClickListener;
    private AdapterView.OnItemSelectedListener onItemSelectedListener;
    private boolean isArrowHidden;
    private int textColor;//NiceSpinner显示的文字颜色
    private int listViewBackgroundSelector;//BaseAdapter中listview的item背景资源
    private int niceSpinnerBackgroundSelector;//NICESpinner选中时背景。
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        listViewBackgroundSelector = typedArray.getResourceId(R.styleable.NiceSpinner_listViewBackgroundSelector, R.drawable.selector);
        niceSpinnerBackgroundSelector= typedArray.getResourceId(R.styleable.NiceSpinner_niceSpinnerBackgroundSelector, R.drawable.nicespinner_bg);
        setBackgroundResource(niceSpinnerBackgroundSelector);
    ...
    public <T> void attachDataSource(List<T> list) {
        adapter = new NiceSpinnerAdapter<>(getContext(), list, textColor, listViewBackgroundSelector,
                spinnerTextFormatter);
        setAdapterInternal(adapter);
    }

    public void setAdapter(ListAdapter adapter) {
        this.adapter = new NiceSpinnerAdapterWrapper(getContext(), adapter, textColor, listViewBackgroundSelector,
                spinnerTextFormatter);
    ...
    private void animateArrow(boolean shouldRotateUp) {
        ...
        if(shouldRotateUp) {
            drawable= ContextCompat.getDrawable(getContext(),R.drawable.ic_sp_ups);
        }else{

            drawable= ContextCompat.getDrawable(getContext(),R.drawable.ic_sp_downs);
        }
